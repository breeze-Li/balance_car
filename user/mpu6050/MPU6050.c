...
MPU6050WRITE_REG等换成你自己的IIC写寄存器的函数即可然后删除掉"_MyI2C_.h"和i2cbus_struct mpu6050_i2cbus;
...
//i2cbus_struct mpu6050_i2cbus; // 创建了一个IIC对象
#define MPU6050_DELAY_MS(ms)                        Delay(ms)
#define MPU6050WRITE_REG(reg, data)                 reg_write(reg, data)                // 可以替换为你的写寄存器函数
#define MPU6050READ_REG(reg)                        reg_read(reg)                       // 可以替换为你的读寄存器函数
#define MPU6050_READ_REG_CONTINUE(reg, len, P_data) My_I2C_RegReadBytes(I2C1,0xD0,  reg, P_data, len) // 可以替换为你的连续读写寄存器函数
//////////////////////////////////////以上为方便移植的接口///////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////
#define SMPLRT_RATE         200
#define SMPLRT_Dt           1.0f/SMPLRT_RATE
...
///////////////////////////////////////////////////////////////////////////////////////////////////
#if MPU6050_USE_Filter == 1                                   // 如果你使用了PT2滤波器(二阶低通滤波器)
...
#if (MPU6050_USE_Filter == 1)                       // 如果使用了滤波器
    // 滤波器进行滤波
...
void MPU6050_init(void/*GPIO_TypeDef *GPIOx, uint16_t SCl, uint16_t SDA*/)
{
//    // 软件iic初始化(支持两个不同的GPIO，但是你要自己改MPU6050_init函数的形参)
//    MyI2C_Init(&mpu6050_i2cbus,
//               GPIOx, SCl,          // scl引脚配置
//               GPIOx, SDA,          // sda引脚配置
//               MPU6050_ADDRESS, 5); // iic初始化(初始化函数里面已经左移地址),延时时间
    //修改为自己的IIC初始化
    hIIC_Init();
    
    MPU6050_InitTypeDef MPU6050_init_Struct;
    MPU6050_init_Struct.SMPLRT_Rate = SMPLRT_RATE;           // 采样率Hz
    ...
    MPU6050_DELAY_MS(360);      // 开机后等待寄存器正常
    for (int i = 0; i < 3; i++) // 软件校准,减少yaw的零点漂移
        if (MPU6050_SoftCalibrate_Z(200) == 0)
            break; // 校准3次,如果校准成功就退出

#if (MPU6050_USE_Filter == 1) // 如果你使用了滤波器(如果你没有我的filter文件那么你将会报错,该滤波器效果较好使用在无人机上)
    // 初始化滤波器
...
//task_register("MPU6050", MPU6050_Test1, SMPLRT_Dt);          /*测试任务, 100HZ*/
